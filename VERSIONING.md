...
## 🚨 ВАЖНО: Обновление версии после КАЖДОЙ задачи

**ПРАВИЛО:** После выполнения ЛЮБОЙ задачи (фича, баг-фикс, изменение UI) немедленно обновлять версию!

**Workflow:**
1. ✅ Задача выполнена → СРАЗУ обновить version.json и config.js
2. 📝 Добавить описание в changelog
3. 🔄 Обновить импорты config.js (v=X → v=X+1)
4. ✨ Пользователь может запросить `/version` и увидеть актуальную версию

**НЕ откладывать обновление версии на потом!** Обновлять сразу после каждой задачи.

**Какую версию инкрементировать:**
- 🆕 Новая фича / UI изменение → MINOR (0.10.0 → 0.11.0)
- 🐛 Баг-фикс / мелкое исправление → PATCH (0.10.0 → 0.10.1)
- При сомнении → MINOR

**Что обновлять:**
1. `/public/js/version.json` - добавить новую запись в changelog
2. `/public/js/config.js` - обновить константу VERSION
3. Все импорты `config.js?v=X` в JS файлах (инкрементировать v)
